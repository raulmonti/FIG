...
SECTION("M2 without resampling (steady-state, RESTART, monolithic, ad hoc thresholds)")
{
	const string nameEngine("restart");
	const fig::ImpFunSpec ifunSpec("concrete_coupled", "auto");
...
SECTION("M2 with resampling (steady-state, RESTART, monolithic, ad hoc thresholds)")
{
	const string nameEngine("restart");
	const fig::ImpFunSpec ifunSpec("concrete_coupled", "auto");
	const string thrAdHoc("6:3,7:2,8:4,9:3,10:3,11:3,12:3,13:3,14:3");
