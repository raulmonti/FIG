...
int main()
{
	...
	try {
		...
		throw TestException(std::string("\n").append(__FILE__).append(":")
							.append(std::to_string(__LINE__)).append(
							": the tests below need access to private class "
							"members. Redeclare those as public, uncomment "
							"the tests code and run again."));
		test_precondition();
		test_postcondition();
		test_transition();
		test_traials();
...
test_label()
{
	std::cout << "Testing fig::Label" << std::endl;
...
test_clock()
{
	std::cout << "Testing fig::Clock" << std::endl;
...
test_variable_interval()
{
	std::cout << "Testing fig::VariableInterval" << std::endl;
...
test_variable_set()
{
	std::cout << "Testing fig::VariableSet" << std::endl;
...
test_state()
{
	std::cout << "Testing fig::State" << std::endl;
...
test_math_expression()
{
	std::cout << "Testing fig::MathExpression" << std::endl;
...
test_precondition()
{
	//  To test, declare "pin_up_vars()" as public in fig::Precondition

//	std::cout << "Testing fig::Precondition" << std::endl;
//
...
test_postcondition()
{
	//  To test, declare "pin_up_vars()" as public in fig::Postcondition

//	std::cout << "Testing fig::Postcondition" << std::endl;
//
...
test_transition()
{
	//  To test, declare "callback()" as public in fig::Transition

//	std::cout << "Testing fig::Transition" << std::endl;
//
...
test_traials()
{
	//  To test, declare "numClocks" and "numVariables" as public in fig::TraialPool

//	std::cout << "Testing fig::TraialPool" << std::endl;
//
...
test_module_instance()
{
	//  To test, declare "mark_added()" and "seal()" as public in fig::ModuleInstance

//	std::cout << "Testing fig::ModuleInstance" << std::endl;
//
//	using fig::Clock;
//	using fig::Label;
//	using fig::ILabel;
...
//
//	fig::Traial& traial = fig::TraialPool::get_instance().get_traial();
//	module1.jump(OLabel("b"), 1.0, traial);
//	try {
//		module2.jump(OLabel("badlabel"), 1.0, traial);  // module2 wasn't sealed
//		throw TestException(to_string(__LINE__).append(": previous statement "
//													   "should have thrown"));
//	} catch (fig::FigException) { /* this was expected */ }
//	fig::TraialPool::get_instance().return_traial(std::move(traial));
}
...
//	auto property = std::make_shared< fig::PropertyTransient >(
//						"q==2", stop_vars, "p==2", goal_vars);
