...
SPLITS=(3 6 9)  # RESTART splittings to test
QUEUES_CAPACITIES=(6 8 10 12 14)
...
RAW_RESULTS=${RESULTS}/RAW_results; #mkdir $RAW_RESULTS
...
for s in "${SPLITS[@]}"; do
	...
	# Still with try our best with bash indirection
	# (http://stackoverflow.com/a/8515492)
	for IFUN in "${IFUNS[@]}"; do
		eval "$IFUN=${IFUN}_s${s}.dat"
		summarize_estimates $RAW_RESULTS EXPERIMENTS[@] $IFUN $S $CONF $PREC \
			&> "e_"${!IFUN}
#		summarize_times     $RAW_RESULTS EXPERIMENTS[@] $IFUN $S $CONF $PREC \
#			&> "t_"${!IFUN}
	done
	gnuplot -e "
		XMIN='${QUEUES_CAPACITIES[0]}';
		XMAX='${QUEUES_CAPACITIES[${#QUEUES_CAPACITIES[*]}-1]}';
		XSTEP=2;
		SPLIT='${s}';
		${IFUNS[0]}='e_${!IFUNS[0]}';
		${IFUNS[1]}='e_${!IFUNS[1]}';
