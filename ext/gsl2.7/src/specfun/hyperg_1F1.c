...
hyperg_1F1_ab_posint(const int a, const int b, const double x, gsl_sf_result * result)
{
  ...
  if(a == b) {
    ...
    result->val = Ma/Mn;
	result->err = 2.0 * GSL_DBL_EPSILON * (fabs((float)a) + 1.0) * fabs(Ma/Mn);
    return stat_CF1;
  }
	...
	result->err += 4.0 * GSL_DBL_EPSILON * (fabs((float)b-a)+1.0) * fabs(result->val);
    ...
    if(b < a) {
      ...
      result->val  = Mn;
	  result->err  = fabs(Mn) * (1.0 + fabs((float)a)) * fabs(r_Mn.err / r_Mn.val);
...
gsl_sf_hyperg_1F1_int_e(const int a, const int b, const double x, gsl_sf_result * result)
{
  ...
  else if(x > 100.0  && GSL_MAX_DBL(1.0,fabs((float)b-a))*GSL_MAX_DBL(1.0,fabs((float)1-a)) < 0.5 * x) {
    /* x -> +Inf asymptotic */
    return hyperg_1F1_asymp_posx(a, b, x, result);
  }
  else if(x < -100.0 && GSL_MAX_DBL(1.0,fabs((float)a))*GSL_MAX_DBL(1.0,fabs((float)1+a-b)) < 0.5 * fabs(x)) {
    /* x -> -Inf asymptotic */
    return hyperg_1F1_asymp_negx(a, b, x, result);
  }
  else if(a < 0 && b < 0) {
